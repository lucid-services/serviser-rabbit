...
Context.prototype.inspectIntegrity = function() {
  var self = this;
  return this._connection.then(function(conn) {
    return new Promise(function(resolve, reject) {
      ...
      function onConnected() {
        if (conn.connection.heartbeat) {
          conn.connection.heartbeater.once('beat', onBeat);
          conn.connection.heartbeater.once('timeout', onTimeout);
          return;
        }
        return resolve();
      }
      ...
      function onBeat() {
        conn.connection.heartbeater.removeListener('timeout', onTimeout);
        return resolve();
      }

      function onTimeout(err) {
        conn.connection.heartbeater.removeListener('beat', onBeat);
        return reject(err || new Error('AMQP socket timeout'));
      }
    });
  });
};
...
function SubSocket(channel, opts) {
  ...
  if (!!opts.persistent && !opts.queue) {
    throw new Error('Durable messages has to be binded to explicit queue - provide required `queue` option');
  }

  var setup = channel.then(function(ch) {
    return ch.assertQueue(self.options.queue || '', {
      exclusive: !!self.options.persistent ? false : true,
      autoDelete: !!self.options.persistent ? false : true,
      durable: !!self.options.persistent
    }).then(function(ok) {
      ...
      return ch.consume(ok.queue, function(msg) {
        ...
        self.push(msg);
      }, {noAck:true, exclusive: !!self.options.persistent ? false : true})
